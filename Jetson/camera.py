...
def grab_img(cam):
    ...
    while cam.thread_running:
        _, cam.img_handle = cam.caps[cam.currentcamnum].read()
...
class Camera():
    ...
    def changecamnum(self, value=0):
        self.currentcamnum = value
    def open(self):
        """Open camera based on command line arguments."""
        assert self.caps[self.currentcamnum] is None, 'Camera is already opened!'
        args = self.args
        if args.use_file:
            self.caps[self.currentcamnum] = cv2.VideoCapture(args.filename)
            # ignore image width/height settings here
            self.use_thread = False
        elif args.use_image:
            self.caps[self.currentcamnum] = 'OK'
            ...
            self.caps[self.currentcamnum] = open_cam_rtsp(
                args.rtsp_uri,
                args.image_width,
                args.image_height,
                args.rtsp_latency
            ...
            self.caps[self.currentcamnum] = open_cam_usb(
                0,
                args.image_width,
                args.image_height
            )
            ...
            self.caps[self.currentcamnum] = open_cam_onboard(
                args.image_width,
                args.image_height
            )
            self.use_thread = True
        if self.caps[self.currentcamnum] != 'OK':
            if self.caps[self.currentcamnum].isOpened():
                # Try to grab the 1st image and determine width and height
                _, img = self.caps[self.currentcamnum].read()
                if img is not None:
                    self.img_height, self.img_width, _ = img.shape
                    self.is_opened = True

    def open_usb(self, camnum, imgwidth, imgheight):
        """Open camera based on command line arguments."""
        #self.caps = []
        #assert self.caps[currentcamnum] is None, 'Camera is already opened!'
        arraysize = len(self.caps)
        self.caps.append(open_cam_usb(
            camnum,
            imgwidth,
            imgheight
        )
        ...
        self.use_thread = True
        if self.caps[arraysize] != 'OK':
            if self.caps[arraysize].isOpened():
                # Try to grab the 1st image and determine width and height
                _, img = self.caps[arraysize].read()
                if img is not None:
                    self.img_height, self.img_width, _ = img.shape
                    self.is_opened = True
    ...
    def read(self):
        if self.args.use_file:
            ...
            if img is None:
                # logging.warning('grab_img(): cap.read() returns None...')
                # looping around
                self.caps[self.currentcamnum].release()
                self.caps[self.currentcamnum] = cv2.VideoCapture(self.args.filename)
                _, img = self.caps[self.currentcamnum].read()
            return img
    ...
    def release(self):
        assert not self.thread_running
        if self.caps[self.currentcamnum] != 'OK':
            self.caps[selfcurrentcamnum].release()
